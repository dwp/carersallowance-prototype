...
router.get(/^\/([^.]+)$/, function (req, res) {
  ...
  res.render(path, merge(true, {session:req.session, cookies:req.cookies}, req.data), function(err, html)
  {
    // if it errors try seeing whether it's a folder
    // name and look for an index.html file inside it.
    if (err)
    {
      res.render(path + "/index", merge(true, {session:req.session, cookies:req.cookies}, req.data), function(err2, html)
      {
        if (err2) {
